...
async function getPostById(id:string):Promise<Post[]> {
    const data = await fetch("https://back-p5-y0e1.onrender.com/api/posts/"+id)
...
async function setComment(id: string,author: string,content: string): Promise<boolean> {
  const data = await fetch(`https://back-p5-y0e1.onrender.com/api/posts/${id}/comments`, {
    method: "POST",
    headers: {
...
export const handler: Handlers = {
   async GET(_req: Request, ctx: FreshContext<unknown, PostProps>) {
    try {
      const {id} = ctx.params
  ...
  async POST(req:Request, ctx) {
    ...
    try {
      await setComment(ctx.params.id,author,content)
...
export default function PostDetail(props : PageProps) {
